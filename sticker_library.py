...
_MAX_TRIGGER_WORDS = (
    3  # el trigger mas largo del catalogo ("cruzar la frontera") tiene 3 palabras
)


def find_keyword_cues(
    words: list[tuple[float, float, str]], min_gap: float = 1.8, max_cues: int = 40
...
def prepare_render_cues(
    ...
    cues = []
    for start, sid, word in find_keyword_cues(
        words, min_gap=min_gap, max_cues=max_cues
    ):
        entry = manifest[sid]
        ...
        cues.append(
            {"time": start, "keyword": word.upper(), "emoji": "", "photo": dst_name}
        )
    return cues
...
def _generate_icon_image_seedream(
    prompt: str, api_key: str, attempts: int = 3
) -> bytes | None:
...
def _generate_icon_image(
    prompt: str, piapi_key: str, attempts: int = 3
) -> bytes | None:
...
def generate_library(
    force: bool = False, only: list[str] | None = None, delay: float = 4.0
) -> None:
    ...
    try:
        ...
        has_rembg = False
        print(
            "AVISO: rembg no instalado (py -m pip install rembg onnxruntime) -- "
    ...
    pending = [
        ...
        for c in todo
        if force
        or c["id"] not in manifest
        or not (STICKERS_DIR / manifest.get(c["id"], {}).get("file", "")).exists()
    ]
    print(
        f"{len(pending)}/{len(todo)} stickers a generar "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Genera/gestiona la libreria de stickers reutilizable"
    )
    parser.add_argument(
        "--generate", action="store_true", help="genera los stickers faltantes"
    )
    parser.add_argument(
        "--force", action="store_true", help="regenera aunque ya existan"
    )
    parser.add_argument("--only", help="ids separados por coma, ej: money,lag,smurf")
    parser.add_argument(
        "--list", action="store_true", help="muestra estado del catalogo"
    )
    ...
    if args.list or not args.generate:
        ...
        for c in STICKER_LIBRARY:
                ...
                c["id"] in manifest
                and (STICKERS_DIR / manifest[c["id"]]["file"]).exists()
            )
            status = "OK   " if done else "falta"
            print(
                f"  [{status}] {c['id']:20} ({c['category']:10}) {', '.join(c['triggers'][:4])}"
            )
        done_n = sum(
            1
            for c in STICKER_LIBRARY
