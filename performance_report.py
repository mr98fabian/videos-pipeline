...
def build_report(only_account: str | None = None) -> list[dict]:
    ...
    for r in rows:
        acc = r.get("account") or "default"  # filas viejas (antes de sumar la columna 'account')
    ...
    for account, acc_rows in by_account.items():
        ...
        try:
            ...
            print(f"[report] fallo al pedir metricas para '{account}': {e} -- se omite esa cuenta")
            continue
        for r in acc_rows:
...
def print_insights(rows: list[dict]) -> None:
    ...
    if with_pct:
        n = max(1, len(with_pct) // 4)  # cuartil en vez de "26 fijo" (ese numero era para 5500 videos)
        by_pct = sorted(with_pct, key=lambda r: _num(r, "averageViewPercentage"), reverse=True)
        top = by_pct[:n]
        bottom = by_pct[-n:]
        top_views = statistics.mean(_num(r, "views") for r in top)
        bottom_views = statistics.mean(_num(r, "views") for r in bottom)
        print(f"  Retencion alta (top {n}, avg %view={statistics.mean(_num(r,'averageViewPercentage') for r in top):.0f}%) "
              f"-> {top_views:,.0f} vistas promedio")
        print(f"  Retencion baja (bottom {n}, avg %view={statistics.mean(_num(r,'averageViewPercentage') for r in bottom):.0f}%) "
    ...
    if len(with_dur) >= 4:
        ...
        print(f"  Videos mas largos  (avg {long_dur:.0f}s) -> {long_avg:,.0f} vistas promedio")
    ...
    for score_field in ("keyword_score", "title_score"):
        ...
        if len(with_score) >= 4:
            by_score = sorted(with_score, key=lambda r: _num(r, score_field), reverse=True)
            half = len(by_score) // 2
...
def main() -> int:
    ...
    if not rows:
        print("[report] video_log.csv vacio o sin videos de esa cuenta -- nada que reportar")
        return 1
    ...
    print(f"[report] {len(rows)} filas -> {out_path} (abrilo en Excel/Sheets para ordenar/filtrar)")
