...
```
Common flags: `--flow` (Google Flow via browser automation — free with the Pro account, and the **only** generator left that accepts a reference image, so the only one that keeps a character identical across scenes; needs `py flow_automation.py --login` once), `--flow-animate` (additionally turns each scene into an 8s image→video clip in Flow — minutes per scene), `--comfy` (ComfyUI + FLUX.1-schnell, local and free, but no reference image), `--seedream` (PiAPI/ByteDance, paid, currently out of credit), `--archivo` (render with the Remotion "Archivo Vivo" engine instead of the classic FFmpeg assembly — requires generated scene images), `--no-pexels`, `--no-sfx`, `--no-motion`, `--watermark ""`, `--hook-max`/`--no-hook-max` (first-2-seconds hook bundle, on by default).
...
- [korex_qa.py](korex_qa.py) — **la puerta de calidad; correrla antes de montar cualquier vídeo KOREX** (`py korex_qa.py output/<carpeta>`, sale con código ≠ 0 si algo falla). Existe porque **todos** los fallos del 4 ago 2026 fueron silenciosos: el generador devolvía OK y el archivo era válido, pero el contenido estaba mal. Comprueba cuatro cosas, una por cada fallo real medido: falta una escena · la "escena" es en realidad la lámina de referencia recién subida · dos escenas son la misma imagen · el clip animado no arranca en su propia imagen (Flow ignora a veces la referencia y genera texto→vídeo: salió un señor fotorrealista donde debía ir Tadeo). Además emite `qa_contactsheet.png` con las escenas arriba y el primer fotograma de su clip debajo — si una columna no casa en vertical, ese clip está mal. Revisar 24 archivos a mano falla; revisar una imagen, no.
- [korex_fill.py](korex_fill.py) / [korex_animate.py](korex_animate.py) — completar escenas que faltan, y animar+montar (voz, música con *ducking*, SFX). Los dos **convergen por rondas insistiendo sólo en lo que falta**, porque Flow falla de forma intermitente sin patrón: reintentar más fuerte dentro de una corrida no basta, y con un intento por escena quedaban 7 de 12 estáticas. `korex_animate` verifica cada clip contra su imagen y **aborta si falta alguna escena** en vez de montar con huecos.
- [flow_automation.py](flow_automation.py) — Playwright browser automation over Google Flow (session persisted in `.flow_profile/` after `--login`, which the user runs, never Claude). Does text→image, **image→image with reference ingredients**, and **image→video**. Two things about it are load-bearing: (a) the hidden `input[type=file]` only uploads to the project library — the model ignores it; a reference only counts once it is added through the `+` picker's *"Agregar a la instrucción"*, and skipping that silently produced a photorealistic raccoon instead of Tadeo; (b) results are downloaded from the tile's direct `media.getMediaUrlRedirect` URL with the session cookies, checking `Content-Type`, because Flow serves a video's poster JPEG from a near-identical URL. Run `py flow_automation.py --inspect` after any Google build change: the UI is a SPA with hashed classes, so every selector is text/aria based and gets revalidated from `.flow_inspect/`.
...
- **Rule 11 — share engineering (9 ago 2026).** The schema requires three new fields per script: `target_emotion` (awe/surprise/humor/outrage — only high-arousal emotions get shared, Berger & Milkman), `twist_phrase` (4-8 verbatim words of the twist sentence, which must land at 30-60% of the script — surprise is the #1 share trigger, Dobele 2007) and `comment_cta` (choice-based question for the pinned comment, never spoken). `_check_giro_posicion` lints emotion + twist position (warn band 25-75%); the full script JSON is saved as `guion.json` in the output dir and `post_question_comment.py` reads `comment_cta` from there. Motivation: median shares = 0 across 51 measured videos (see `ANALISIS_OUTLIERS_PROPIOS.md`).
...
- **The 9 ago 2026 multi-language research corpus** (EN/ES/ZH/JA/KO sources, each doc ends with a pipeline-application section): `PSICOLOGIA_COMPARTIR.md` (STEPPS, high-arousal emotions, social currency), `ANALISIS_OUTLIERS_PROPIOS.md` (our own 51-video data — 80-120 word sweet spot, shares median 0; **our measured data overrides any external benchmark**), `SOUND_DESIGN_RETENCION.md` (ducking params, 2-3s cuts, SFX before key moments, -14 LUFS/-1.0 dBTP), `VOZ_TTS_EMOCIONAL.md` (one voice per channel forever, emotion in dosis), `MINIATURAS_SHORTS_2026.md` (thumbnail = 0% in feed, +140% browse, +85% search), `HUMOR_LOCALIZACION_LATAM.md` (4 humor styles by culture, punch-up rule), `SERIES_LORE_PERSONAJES.md` (character IP, Regular viewers KPI), `COMENTARIOS_ENGAGEMENT.md` (thread depth, 2-6h reply window, choice CTAs). `INVESTIGACION_MULTIIDIOMA_2026.md` is the parent synthesis.
